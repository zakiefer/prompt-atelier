...
### Forced dark theme
No light mode toggle. body gets bg-bg text-text-primary.
...
Contents:
1. Logo: 9x9 circle with accent gradient border, inner bg-bg circle with "JA" in font-display italic text-[13px]. Scales 110% on hover.
...
4. Divider
5. "Say hi" button with accent gradient border on hover and arrow.
...
- Role line: "A {role} lives in Chicago." - roles cycle every 2s through ["Creative", "Fullstack", "Founder", "Scholar"]. Role word uses font-display italic text-text-primary animate-role-fade-in inline-block with key={roleIndex}.
...
- CTA Buttons: "See Works" solid button and "Reach out..." outlined button. Both rounded-full text-sm px-7 py-3.5 hover:scale-105 with accent gradient hover border ring.
...
Each card: bg-surface border border-stroke rounded-3xl with aspect ratios. Contains background image, halftone overlay, hover bg-bg/70 backdrop-blur-lg overlay, and hover label pill with animated gradient border.
...
4 journal entries displayed as horizontal pills (rounded-[40px] sm:rounded-full) with titles, images, read times, and dates. Each entry: flex items-center gap-6 p-4 bg-surface/30 hover:bg-surface border border-stroke.
